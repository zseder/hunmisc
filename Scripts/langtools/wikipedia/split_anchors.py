...
def filter(filter_files):
    ...
    for filter_file in filter_files:
        fin = FileReader(filter_file).open()
        for line in fin:
...
if __name__ == '__main__':
    try:
        params, args = cmd_utils.get_params(sys.argv[1:], 'w:t:m:f:n:g:s:', 'wt', 0)
    ...
    for infile in params['i']:
        fin = FileReader(infile).open()
        good_out = FileWriter(infile + 'good', 'w').open()
        bad_out = FileWriter(infile + 'bad', 'w').open()
        curr_out = good_out
        for line in fin:
            l = line.strip()
